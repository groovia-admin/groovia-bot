...
interface CreatedOwnerCredentials {
  email: string;
...
export default function ShopsClient({
  ...
  async function toggleActive(shop: ShopRow) {
    ...
    setShops((p) =>
      p.map((s) => (s.id === shop.id ? { ...s, is_active: next } : s)),
    );
  ...
  async function updateSub(shop: ShopRow, status: SubscriptionStatus) {
    ...
    setShops((p) =>
      p.map((s) =>
        s.id === shop.id ? { ...s, subscription_status: status } : s,
      ),
    );
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      ...
      setShops((p) => [result.shop as ShopRow, ...p]);
      setCreatedOwner({
      ...
      showToast(`Shop "${result.shop.name}" created`);
    } catch (error) {
      setAddError(
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      {/* Filters */}
      <div className="shops-filters" style={{ display: "flex", gap: 12 }}>
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: 12, color: "#94a3b8", marginBottom: 20 }}>
              Creates a 60-day trial account and an owner login.
            </p>
            {createdOwner ? (
              <div
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ ...S.input, wordBreak: "break-all" }}>
                    {createdOwner.email}
                  </div>
                </div>
                <div>
                ...
                <p style={{ fontSize: 11, color: "#fbbf24", lineHeight: 1.5 }}>
                  Save these credentials now. This temporary password is not
                  stored anywhere in the dashboard after this window is closed.
                </p>
                <button
                  type="button"
                  onClick={() => {
